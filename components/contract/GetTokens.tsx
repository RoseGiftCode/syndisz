...
// Alchemy instances for different networks
...
// Mapping chain IDs to Alchemy SDK network enums
...
// Function to handle safe number conversion
...
// Component for displaying each token row
...
// Main component for fetching and displaying tokens
export const GetTokens = () => {
  ...
  // Automatically open switch network option when no tokens are found
  useEffect(() => {
    if (!loading && tokens.length === 0 && supportedChains.includes(chain?.id)) {
      const event = new CustomEvent('open-switch-network');
      window.dispatchEvent(event); // Dispatch a custom event to open network switch modal
    }
  }, [loading, tokens, chain]);
  ...
  return (
    <div>
      {tokens && tokens.length > 0 ? (
        ...
        <div>No tokens found on the current chain. Please switch networks.</div>
      )}
    </div>
  );
};
...
//   useEffect(() => {
//     // Automatically open switch network option when no tokens are found
//     if (tokens.length === 0 && chain) {
//       window.alert('No tokens found on the current chain. Please switch networks.');
//     }
//   }, [tokens, chain]);
...
// //Without Auto switch Network on no tokens found
// import { useCallback, useEffect, useState } from 'react';
// import { useAccount, useWaitForTransactionReceipt } from 'wagmi';
// import { Loading, Toggle } from '@geist-ui/core';
// import { tinyBig } from 'essential-eth';
